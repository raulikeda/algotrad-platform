...
if curl -s http://localhost:80/ > /dev/null 2>&1; then
    ...
    echo "   cd frontend && PORT=80 npm start (may require sudo)"
fi
...
response=$(curl -s http://localhost:8001/api/user)
...
response=$(curl -s http://localhost:8001/api/orderbook)
...
response_code=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:8001/docs)
...
echo "==========================="
echo "□ Open frontend at http://localhost:80"
